"""
Terminus (C4) agent data extractor - extracts from response.json format.
"""
import json
import traceback
...
class TerminusDataExtractor:
    """Extract task data from Terminus (C4) agent run directory."""
    ...
    def _extract_episode(self, episode_dir: Path, episode_num: int) -> Optional[EpisodeData]:
        """Extract data from a single episode (Terminus/C4 format: response.json)."""
        try:
            prompt_file = episode_dir / "prompt.txt"
            response_file = episode_dir / "response.json"  # Terminus uses response.json
            ...
            if response_file.exists():
                ...
                try:
                    ...
                    # Map Terminus field names to standard names
                    analysis = response_data.get("state_analysis")  # state_analysis → analysis
                    plan = response_data.get("explanation")  # explanation → plan
