#!/usr/bin/env python3
"""
Test script for Hermes agent analysis.

Usage:
  # Generate prompt only
  python tests/test_hermes.py --run-dir /path/to/task --tasks-dir /path/to/tasks --mode prompt

  # Run full analysis
  python tests/test_hermes.py --run-dir /path/to/task --tasks-dir /path/to/tasks --mode analyze \
...
from agents.hermes.data_extractor import HermesDataExtractor
from agents.hermes.analyze_failures import AnalysisPromptBuilder, FailureAnalyzer
from src.llm_providers import get_provider
from src.output_generator import StreamingOutputGenerator


def main():
    parser = argparse.ArgumentParser(description="Test Hermes agent analysis")
    ...
    if args.mode == "prompt":
        ...
        output_dir = Path("./test_output_hermes")
